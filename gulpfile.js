...
gulp.task('format-root', function(callback) {
	return gulp.src(['*.js']).pipe(formatFile()).pipe(gulp.dest('.'));
});

gulp.task('format-src', function(callback) {
	return gulp.src(['src/**/*.js']).pipe(formatFile()).pipe(gulp.dest('src'));
});
...
gulp.task('config', function() {
	return gulp.src('src/config/**/*.*').pipe(gulp.dest('dist/demo/config'));
});

gulp.task('copy-bower', function() {
	return gulp
		.src('src/modules/bower.json')
...
gulp.task('demo', function() {
	return gulp.src('src/demo/**/*.*').pipe(gulp.dest('dist/demo'));
});

gulp.task('create-loader', ['create-loader-debug'], function() {
	return gulp
		.src('dist/loader-debug.js')
		.pipe(stripDebug())
...
gulp.task('create-loader-debug', ['create-loader-pure-debug'], function() {
	return gulp
		.src('src/template/loader.template')
		.pipe(
			template({
				vendor: fs
					.readFileSync(
...
gulp.task(
	'create-loader-min',
	['create-loader', 'create-loader-pure-min', 'copy-es6promise-map'],
	function() {
		return gulp
			.src('dist/loader.js')
			.pipe(uglify())
...
gulp.task(
	'create-loader-pure-debug',
	['create-loader-pure-wrapped'],
	function() {
		var loaderPureContent = fs
			.readFileSync('dist/loader-pure-wrapped.js')
			.toString();

		fs.unlinkSync('dist/loader-pure-wrapped.js');

		return gulp
			.src('src/template/loader-pure.template')
			.pipe(
				template({
...
gulp.task('copy-es6promise-map', function() {
	return gulp
		.src(['node_modules/es6-promise/dist/es6-promise.map'])
		.pipe(gulp.dest('dist'));
});
...
gulp.task(
	'modules',
	['copy-bower', 'modules2', 'modules3', 'packages'],
	function() {
		return gulp
			.src('src/modules/**/*.js')
			.pipe(
...
gulp.task('test', ['build'], function() {
	var streamStripDebug = gulp
		.src(['umd/**/*.js', '!umd/event-emitter.js'])
		.pipe(stripDebug());
...
gulp.task('wrap-config-parser', function() {
	return gulp
		.src('src/template/config-parser.template')
		.pipe(
			template({
...
gulp.task('wrap-event-emitter', function() {
	return gulp
		.src('src/template/event-emitter.template')
		.pipe(
			template({
...
gulp.task('wrap-dependency-builder', function() {
	return gulp
		.src('src/template/dependency-builder.template')
		.pipe(
			template({
				source: fs
					.readFileSync('src/js/dependency-builder.js')
					.toString(),
			})
		)
...
gulp.task('wrap-script-loader', function() {
	return gulp
		.src('src/template/script-loader.template')
		.pipe(
			template({
...
gulp.task('wrap-path-resolver', function() {
	return gulp
		.src('src/template/path-resolver.template')
		.pipe(
			template({
...
gulp.task('wrap-url-builder', function() {
	return gulp
		.src('src/template/url-builder.template')
		.pipe(
			template({
